<!DOCTYPE html>
<html lang="en-gb" dir="ltr" class="uk-height-1-1">
    <head>
        <meta charset="utf-8">
        <title>用户登入</title>
        <link rel="stylesheet" href="/uikit/css/uikit.gradient.min.css" />
    </head>
    <body class="uk-height-1-1">
        <div class="uk-vertical-align uk-text-center uk-height-1-1">
            <div class="uk-vertical-align-middle" style="width: 250px;">
                <img class="uk-margin-bottom" width="140" height="120" src="data:image/svg+xml;base64," alt="">
                <?php echo Form::open(array('url' => '/login','class' => 'uk-panel uk-panel-box uk-form','method' => 'post','id'=> 'login_form'));?>
                    <div class="uk-form-row">
                        <?php echo Form::text('username',null,array('class' => "uk-width-1-1 uk-form-large",'placeholder' => "Username"));?>
                    </div>
                    <div class="uk-form-row">
                        <?php echo Form::password('password',array('class' => "uk-width-1-1 uk-form-large",'placeholder' => "Password"));?>
                    </div>
                    <?php if($msg):?>
                    <div class="uk-alert uk-alert-danger">
                        <?php echo $msg;?>
                    </div>
                    <?php endif;?>
                    <div class="uk-form-row">
                        <a class="uk-width-1-1 uk-button uk-button-primary uk-button-large" href="javascript:$('#login_form').submit();">Login</a>
                    </div>
                    <div class="uk-form-row uk-text-small">
                        <label class="uk-float-left">
                            <?php echo Form::checkbox('remember_me', '1');?> Remember me</label>
                        <a class="uk-float-right uk-link uk-link-muted" href="#">忘记密码？</a>
                    </div>
                <?php echo Form::close();?>
            </div>
        </div>
<script type="text/javascript" src="//lib.sinaapp.com/js/jquery/2.0.2/jquery-2.0.2.min.js"></script>
<script type="text/javascript" src="/uikit/js/uikit.min.js"></script>
</body>
</html>